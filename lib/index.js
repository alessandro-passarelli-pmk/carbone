...
/**
 * Inject files (images, barcodes, …) that were queued by formatters into the
 * template file list and, for ODF documents, update META-INF/manifest.xml.
 *
 * @param  {Object} template      template object (modified in place)
 * @param  {Array}  pendingFiles  [{name, data, mimeType, parent}] collected from options.pendingFiles
 */
function injectPendingFiles (template, pendingFiles) {
  if (!pendingFiles || pendingFiles.length === 0) {
    return;
  }

  // Locate the ODF manifest so we can register each new file
  var _manifestIndex = -1;
  for (var i = 0; i < template.files.length; i++) {
    if (template.files[i].name === 'META-INF/manifest.xml') {
      _manifestIndex = i;
      break;
    }
  }
  ...
  for (var j = 0; j < pendingFiles.length; j++) {
    var _pf = pendingFiles[j];

    // Add the binary file to the template so the ZIP assembler picks it up
    template.files.push({
      name     : _pf.name,
      data     : _pf.data,
      isMarked : false,
    ...
    // Register in META-INF/manifest.xml (ODF formats only)
    if (_manifestIndex >= 0) {
      var _entry = ' <manifest:file-entry manifest:full-path="' + _pf.name +
        '" manifest:media-type="' + _pf.mimeType + '"/>\n';
      var _manifest = template.files[_manifestIndex].data;
      template.files[_manifestIndex].data = _manifest.replace(
        '</manifest:manifest>',
...
function walkFiles (template, data, options, currentIndex, callback) {
  if (currentIndex >= template.files.length) {
    // Inject any files (images, barcodes, …) queued by formatters during XML processing
    if (options.pendingFiles && options.pendingFiles.length > 0) {
      injectPendingFiles(template, options.pendingFiles);
...
carbone.addFormatters(require('../formatters/image.js'));
